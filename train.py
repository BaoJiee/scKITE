...
# Avoid Hugging Face tokenizer fork warnings when DataLoader uses workers.
...
def unwrap_model(model: torch.nn.Module) -> TahoeStage2MixedModel:
...
def build_model(
    ...
    model = TahoeStage2MixedModel(**model_cfg)
...
def train_one_epoch(
    ...
    # Critical for epoch-aware cyclic_without_replacement.
    collator = getattr(
        loader,
        "collate_fn",
        None,
    ...
    for batch in pbar:
        ...
        if (
            ...
            # GradScaler lowers scale when an overflow causes optimizer.step to be skipped.
            optimizer_was_run = (
                scale_after
                >= scale_before
            )
        ...
        # -------------------------------------------------------------
        # Only successful optimizer updates count as global_step.
        # -------------------------------------------------------------
        if not optimizer_was_run:
            continue
        ...
        # -------------------------------------------------------------
        # DDP-average current-step scalar losses.
        # These values enter the rolling-100 window.
        # -------------------------------------------------------------
        ...
        # W&B logs every successful optimizer step by default.
        if (
            wandb_log_every > 0
            and global_step
            % wandb_log_every
        ...
        if (
            ...
            if is_main_process():
                ...
                if (
                    ...
                    # Only the compact validation metric set is logged.
                    wandb.log(
                        format_wandb_metrics(
                            step_metrics
                        ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train scKODE Stage2 without Regulon Activity Head: "
...
def main() -> None:
    ...
    # ---------------------------------------------------------
    # Stage2 differential learning rates
    #   Encoder / Stage1-inherited modules -> encoder_lr
    #   Regulon + Annotation decoder modules -> decoder_lr
    #
    # shared_token_embedding is intentionally kept in the
    # Encoder group because it is inherited from Stage1,
    # although both decoders also use this shared embedding.
    # ---------------------------------------------------------
    encoder_lr = float(
        train_cfg.get(
            "encoder_lr",
    ...
    try:
        for epoch in range(
            ...
            if is_main_process():
                ...
                # Epoch-end W&B logs only compact validation metrics.
                if (
                    val_loader is not None
                    and wandb is not None
                    and wandb.run
