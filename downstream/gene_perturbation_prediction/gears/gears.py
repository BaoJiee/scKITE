...
class GEARS:
    ...
    def __init__(self, pert_data, 
        ...
        self.gene_list = pert_data.gene_names.values.tolist()  # 保存 GEARS gene symbol 列表。 #
        self.gene_id_list = pert_data.adata.var.index.astype(str).tolist()  # 保存 GEARS var.index，通常是 Ensembl ID。 #
        self.pert_list = pert_data.pert_names.tolist()  # 保存 GEARS perturbation gene 列表。 #
        ...
        self.ctrl_expression = torch.tensor(
            np.mean(self.adata.X[np.asarray(self.adata.obs.condition == 'ctrl')],
    ...
    def tunable_parameters(self):
        ...
        return {
            ...
            'coexpress_threshold': 'pearson correlation threshold when constructing co-expression graph, default 0.4',
            'uncertainty': 'whether or not to turn on uncertainty mode, default False',
            'uncertainty_reg': 'regularization term to balance uncertainty loss and prediction loss, default 1',
            'direction_lambda': 'regularization term to balance direction loss and prediction loss, default 1e-1',
            'adapter_name': 'adapter name: native_gears or a registered model adapter',
            'gene_embedding_mode': 'gene embedding mode: native, scfm_static, scfm_contextual, sckite_static, or sckite_contextual',
            'pert_embedding_mode': 'perturbation embedding mode: native, scfm_init, or sckite_init',
            'freeze_pert_emb': 'whether to freeze perturbation embedding after SCFM initialization',
            'num_bins': 'number of bins for contextual expression value binning',
            'contextual_value_mode': 'contextual expression value mode: bin, as_is, or log1p',
            'contextual_max_genes': 'maximum number of genes sent into Stage2 contextual encoder',
            'contextual_gene_selection': 'gene selection strategy for contextual encoder: matched_first or first',
            'contextual_fallback': 'fallback embedding for genes not sent into contextual encoder: static or zero',
        }
    ...
    def model_initialize(self, hidden_size = 64,
                         ...
                         # 新增：adapter 相关参数
                         adapter = None,
                         adapter_name = 'native_gears',
                         adapter_kwargs = None,
                         gene_embedding_mode = 'native',
                         pert_embedding_mode = 'native',
                         freeze_pert_emb = False,

                         # 新增：Stage2 contextual 参数
                         num_bins = 51,
        ...
        # -----------------------------
        # Build / setup SCFM adapter
        # -----------------------------
        adapter_kwargs = dict(adapter_kwargs or {})  # baseline 时 adapter_kwargs=None，转为空字典。 #

        if (
            adapter_name == "sckite"
            or gene_embedding_mode
            in ["scfm_static", "scfm_contextual", "sckite_static", "sckite_contextual"]
            or pert_embedding_mode in ["scfm_init", "sckite_init"]
        ):
            adapter_kwargs.setdefault("gears_hidden_size", hidden_size)  # 保证 adapter 输出维度和 GEARS hidden_size 一致。 #
            adapter_kwargs.setdefault("num_bins", num_bins)  # 传入表达量 bin 数。 #
            adapter_kwargs.setdefault("contextual_value_mode", contextual_value_mode)  # 传入 contextual 表达量处理方式。 #

            if contextual_max_genes is not None and int(contextual_max_genes) > 0:  # 如果设置了最大 gene 数。 #
                adapter_kwargs.setdefault("contextual_max_genes", int(contextual_max_genes))  # 传入最大 gene 数。 #
            else:  # 如果没有限制 gene 数。 #
                adapter_kwargs.setdefault("contextual_max_genes", None)  # None 表示全部 gene。 #

            adapter_kwargs.setdefault("contextual_gene_selection", contextual_gene_selection)  # 传入 gene 选择策略。 #
            adapter_kwargs.setdefault("contextual_fallback", contextual_fallback)  # 传入 fallback 策略。 #
        
        need_adapter = (
            gene_embedding_mode in ['scfm_static', 'scfm_contextual', 'sckite_static', 'sckite_contextual']
            or pert_embedding_mode in ['scfm_init', 'sckite_init']
            or adapter is not None
            or adapter_name != 'native_gears'
        )
        
        if adapter is None and need_adapter:
            from adapters.registry import build_adapter
            adapter = build_adapter(adapter_name, **adapter_kwargs)
        
        if adapter is not None:
            adapter.setup(  # 初始化 adapter 对齐关系。 #
                gene_list=self.gene_list,  # 传入 GEARS gene symbol。 #
                pert_list=self.pert_list,  # 传入 GEARS perturbation gene。 #
                gene_id_list=self.gene_id_list,  # 传入 GEARS var.index / Ensembl ID。 #
            )  # adapter setup 结束。 #
        ...
        self.config = {'hidden_size': hidden_size,
                       ...
                       'scfm_dim': getattr(adapter, 'output_dim', None) if adapter is not None else None,
                       'adapter_name': getattr(adapter, 'name', adapter_name) if adapter is not None else adapter_name,

                       # 新增：记录 contextual 设置，便于复现实验
                       'num_bins': num_bins,
        ...
        # baseline 兼容：原始 GEARS_Model 不支持 adapter 参数。
        import inspect
        if "adapter" in inspect.signature(GEARS_Model.__init__).parameters:
            self.model = GEARS_Model(self.config, adapter=adapter).to(self.device)
        else:
            if need_adapter or adapter is not None:
                raise RuntimeError(
                    "当前 gears/model.py 的 GEARS_Model 不支持 adapter，"
                    "但你开启了 adapter/scFM 模式。请使用支持 adapter 的 model.py。"
                )
            self.model = GEARS_Model(self.config).to(self.device)
        
        if pert_embedding_mode in ['scfm_init', 'sckite_init']:
            if adapter is None:
                raise ValueError(f"pert_embedding_mode='{pert_embedding_mode}' requires an adapter.")
        
            raw_pert_emb = adapter.get_pert_embeddings(self.pert_list)
        
            self.model.initialize_pert_embeddings_from_scfm(
                raw_pert_emb=raw_pert_emb,
                freeze_pert_emb=freeze_pert_emb,
            )
        
        self.best_model = deepcopy(self.model)
        
    def load_pretrained(self, path, adapter=None, adapter_name='native_gears', adapter_kwargs=None):
        """
