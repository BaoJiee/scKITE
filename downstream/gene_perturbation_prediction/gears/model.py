...
class GEARS_Model(torch.nn.Module):
    def __init__(self, args, adapter=None):
        ...
        if self.gene_embedding_mode in ["scfm_static", "scfm_contextual", "sckite_static", "sckite_contextual"]:  # 如果使用外部基模 gene embedding。 #
            ...
            if self.gene_embedding_mode in ["scfm_static", "sckite_static"]:  # 如果使用静态 gene embedding。 #
    ...
    def get_gene_embedding_for_batch(self, data, num_graphs):
        ...
        if self.gene_embedding_mode in ["scfm_static", "sckite_static"]:
        ...
        if self.gene_embedding_mode in ["scfm_contextual", "sckite_contextual"]:
            if self.adapter is None:
                raise ValueError(f"{self.gene_embedding_mode} requires an adapter.")
